...
with st.sidebar:
    ...
    selected_reports = st.multiselect(
        ...
        options=all_reports,
        default=['Статистика по отобранным и неотобранным артикулам', 'Неотобранные артикулы с разбивкой на подтипы']
    )
    ...
    # with st.expander("ℹ️ Текущие настройки"):
    #     st.write(f"**Колонки:** {col_layout}")
    #     st.write(f"**Графики:** {'✅' if show_charts else '❌'}")
    #     st.write(f"**Статистика:** {'✅' if show_stats else '❌'}")
    #     st.write(f"**Таблицы:** {'✅' if show_data else '❌'}")
    #     st.write(f"**Отчетов выбрано:** {len(selected_reports)}")
    #     if selected_employee:
    #         st.write(f"**Сотрудник:** {selected_employee}")
